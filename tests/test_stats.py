...
from src.stats import RunStats, fetch_recent_runs, save_run
...
class TestSaveRun:
    ...
    def test_inserts_row_with_stats_fields(self):
        ...
        assert insert_call["fetched"] == 5
        assert insert_call["delivered"] == 2
        assert insert_call["duration_seconds"] == 9.9
    ...
    def test_does_not_raise_on_supabase_error(self):
        ...
        with patch(PATCH_CLIENT, return_value=client):
            save_run(RunStats())  # must not raise
